...
export default function LegalGuidePage() {
  ...
  const legData = sections.legal_legislation?.content as
    | {
        title?: string;
        icon?: string;
        cards?: {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-12">
          ...
          <p className="text-zinc-400 text-sm leading-relaxed max-w-2xl">
            Kurye haklarıyla ilgili yasal bilgilere erişin, kısıtlamaları nasıl
            raporlayacağınızı öğrenin ve kampanyayı desteklemek için kaynakları
            indirin.
          </p>
        </div>
          ...
          <section className="mb-14">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-9 h-9 rounded-xl bg-purple-500/10 flex items-center justify-center">
                <span className="text-lg">{legData?.icon || "📜"}</span>
              </div>
              <h2 className="text-xl font-bold text-white">
                {legData?.title || "Mevzuat Özeti"}
              </h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {(legData?.cards || []).map((card, i) => (
                ...
                >
                  <span
                    className="inline-block text-[10px] font-semibold uppercase tracking-wider px-2.5 py-0.5 rounded-md mb-3"
                    style={{
          ...
          <section className="mb-14">
            <div className="flex items-center justify-between mb-6">
              ...
              <span className="text-xs text-zinc-600">
                {petitionExamples.length} şablon
              </span>
            </div>
            <div className="space-y-3">
              {petitionExamples.map((petition, i) => (
                <div
                  key={i}
                  className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-4 md:p-5 flex flex-col md:flex-row md:items-center gap-4 hover:border-zinc-700 transition-colors"
                >
                  ...
                  <div className="flex items-center gap-2 pl-13 md:pl-0 shrink-0">
                    {petition.formats.map((fmt) => (
                      <button
                        key={fmt}
                        className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${fmt === "PDF" ? "bg-red-500/15 text-red-400 hover:bg-red-500/25 border border-red-500/20" : "bg-blue-500/15 text-blue-400 hover:bg-blue-500/25 border border-blue-500/20"}`}
                      >
                        {fmt}
                      </button>
                    ))}
          ...
          <section className="mb-14">
            ...
            <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl overflow-hidden">
              ...
              {downloadableResources.map((res, i) => (
                <div
                  key={i}
                  className={`flex flex-col md:grid md:grid-cols-4 gap-2 md:gap-4 px-5 py-4 md:items-center ${i < downloadableResources.length - 1 ? "border-b border-zinc-800/60" : ""}`}
                >
                  ...
                  <div className="flex items-center gap-2">
                    <span
                      className={`text-[10px] font-semibold px-2 py-0.5 rounded ${res.type === "PDF" ? "bg-red-500/15 text-red-400" : "bg-amber-500/15 text-amber-400"}`}
                    >
                      {res.type}
                    </span>
